...
NEW_BORDER_COLOR = (0, 0, 0, 255)  # Black with full opacity (RGBA)
...
def is_border_dark(border_pixels, alpha_pixels=None, border_name="unknown", image_name="unknown"):
    ...
    try:
        ...
        if valid_pixels.ndim == 3 and valid_pixels.shape[-1] == 3:
            ...
            logger.error(f"{image_name} - {border_name} border: Invalid pixel shape: {border_pixels.shape}")
            return False
...
def add_black_border(image_path):
    """
    Add a 2-pixel black border to the image without modifying original content.
    ...
    try:
        ...
        try:
            ...
            logger.debug(f"{image_name} - Original mode: {original_mode}, Size: {pil_img.size}")
        except Exception as e:
            logger.error(f"PIL failed to open {image_path}: {str(e)}")
            return False

        # Add border using PIL
        try:
            ...
            if original_mode == 'L':
                ...
                has_transparency = pil_img.info.get('transparency') is not None or 'tRNS' in pil_img.info.get('pnginfo', {})
                pil_img = pil_img.convert('RGBA' if has_transparency else 'RGB')
                border_color = NEW_BORDER_COLOR if pil_img.mode == 'RGBA' else (0, 0, 0)
            ...
            # Add border
            bordered_img = ImageOps.expand(pil_img, border=NEW_BORDER_SIZE, fill=border_color)
            logger.debug(f"{image_name} - Added {NEW_BORDER_SIZE}-pixel border, New size: {bordered_img.size}")

            # Save with original metadata
            for attempt in range(3):
                try:
                    with open(image_path, 'wb') as f:
                        bordered_img.save(f, format='PNG', compress_level=9, optimize=True, **pil_img.info)
                    logger.debug(f"Saved bordered image to {image_path}")
                    return True
                except Exception as e:
