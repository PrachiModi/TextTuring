...
def validate_chapter_toc(ditamap_path: str) -> list:
    ...
    try:
        ...
        for chapter in chapters:
            ...
            for ul in xml_root.findall(".//ul"):
                ...
                for li in ul.findall("./li"):
                    ...
                    if xref is not None and xref.get('href'):
                        ...
                        # Strip fragment ID (e.g., #xx1121647)
                        href = href.split('#')[0]
            ...
            logger.debug(f"Listed raw hrefs: {[urllib.parse.unquote(x.get('href')).strip().split('#')[0] for x in xml_root.findall('.//ul/li//xref[@href]')]}")
...
def validate_subchapter_toc(ditamap_path: str) -> list:
    ...
    Prints detailed debug output to console for each parent topic.
    ...
    print(f"\n=== Subchapter TOC Validation Started: {ditamap_path} ===\n")
    ...
    try:
        ...
        ditamap_parent = str(Path(ditamap_path).parent)
        logger.debug(f"DITA map parent dir: {ditamap_parent}")
        print(f"DITA map parent directory: {ditamap_parent}")
        ...
        print(f"Found {len(parent_topicrefs)} parent topicrefs with nested topicrefs\n")
        ...
        for parent_topicref in parent_topicrefs:
            ...
            if not parent_href:
                ...
                print("ERROR: Parent topicref missing href attribute")
                continue
            ...
            print(f"\n=== Processing Parent Topic: {parent_name} ===\nXML path: {parent_xml_path}\n")
            ...
            try:
                ...
                print(f"ERROR: Failed to parse parent XML: {parent_xml_path}\nReason: {e}\n")
                continue
            except FileNotFoundError:
                logger.debug(f"Parent XML file not found: {parent_xml_path}")
                print(f"ERROR: Parent XML file not found: {parent_xml_path}\n")
                continue
            ...
            if not has_bulleted_list:
                ...
                print(f"No bulleted lists with xrefs found in {parent_xml_path}\n")
                continue
            ...
            print(f"Expected subtopics (from DITA map <topicref>s):\n{list(topicref_hrefs.keys())}\n")
            print(f"Raw topicref hrefs:\n{list(topicref_hrefs.values())}\n")

            if not topicref_hrefs:
                logger.debug(f"No nested topicrefs found in parent topicref")
                print(f"No nested topicrefs found for {parent_name}\n")
                continue
            ...
            for ul in xml_root.findall(".//ul"):
                for li in ul.findall("./li"):
                    xref = li.find(".//xref")
                    if xref is not None and xref.get('href'):
                        raw_href = xref.get('href')
                        raw_xref_hrefs.append(raw_href)
                        href = urllib.parse.unquote(raw_href).strip()
                        # Strip fragment ID
                        href = href.split('#')[0]
                        xref_hrefs.append(os.path.basename(href))

            logger.debug(f"Listed subtopics in XML: {xref_hrefs}")
            print(f"Found subtopics (from XML <xref>s):\n{xref_hrefs}\n")
            print(f"Raw xref hrefs (before processing):\n{raw_xref_hrefs}\n")

            # Find missing subtopics
            missing_hrefs = [href for href in topicref_hrefs if href not in xref_hrefs]
            logger.debug(f"Missing subtopics: {missing_hrefs}")
            print(f"Missing subtopics:\n{missing_hrefs}\n")
            if missing_hrefs:
                print("Reasons for missing subtopics:")
                for href in missing_hrefs:
                    print(f"- {href}: Not found in xref_hrefs {xref_hrefs}")
                print()

            for href in missing_hrefs:
                topic_path = os.path.normpath(os.path.join(ditamap_parent, topicref_hrefs[href]))
                logger.debug(f"Fetching title for missing subtopic: {topic_path}")
                print(f"Fetching title for missing subtopic: {topic_path}\n")
                try:
                    topic_tree = ET.parse(topic_path)
                    title_elem = topic_tree.find(".//title")
        ...
        print(f"\n=== Subchapter TOC Validation Complete ===\nIssues found: {len(results)}\n")
        if not results:
